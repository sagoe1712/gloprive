...
$(document).on('pageInit', function (e) {
	...
	$(".swipebox").swipebox();
	/*});*/
	$('#notpage').load('http://gloprive.com/mobi/notification.php');
...
//	$('#merchantpage').load('merchantoffers.php');
	...
	$('#showmerlist').on('click', '.merpage', function() {
		...
		$.ajax({
									type: "GET",
									url: "http://gloprive.com/mobi/merchantoffers.php",
									data: {id:id},
...
if ($('#loading_image').length == 0) { //is the image on the form yet?
    ...
    $('.validatebtn').hide(); // disable double submits
		$.ajax({
											type: "POST",
											url: "http://gloprive.com/mobi/chpscript.php",
...
$.ajax({
									type: "POST",
									url: "http://gloprive.com/mobi/genvoucher.php",
